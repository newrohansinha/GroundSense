...
serve(async (req) => {
  ...
  try {
    ...
    // Direction-split totals over the FULL published set (the top-3 risk fetch above is
    // for the narrative blocks only). Downside risk and favorable relief are reported
    // separately and never merged into one "value at stake" figure. Same gate filter.
    const { data: allPublishedRows } = await supabase
      .from("risk_register")
      .select("impact_low, impact_high, issue_direction")
      .eq("company_id", companyId)
      .in("display_section", ["risk_register", "operating_changes"])
      .eq("gate_status", "published");

    const isFavorable = (d: unknown) =>
      d === "favorable" || d === "favorable_with_residual_exposure";
    const publishedRows = allPublishedRows || [];
    const downsideRows = publishedRows.filter((r: any) => !isFavorable(r.issue_direction));
    const favorableRows = publishedRows.filter((r: any) => isFavorable(r.issue_direction));

    const downsideLow = downsideRows.reduce((s: number, r: any) => s + Number(r.impact_low || 0), 0);
    const downsideHigh = downsideRows.reduce((s: number, r: any) => s + Number(r.impact_high || 0), 0);
    const favorableLow = favorableRows.reduce((s: number, r: any) => s + Number(r.impact_low || 0), 0);
    const favorableHigh = favorableRows.reduce((s: number, r: any) => s + Number(r.impact_high || 0), 0);
    const hasFavorable = favorableRows.length > 0 && (favorableLow > 0 || favorableHigh > 0);

    // Retained for the per-risk narrative blocks below (top-3 downside-oriented set).
    const totalRiskLow = downsideLow;
    const totalRiskHigh = downsideHigh;
...
Modeled downside across the published risks is ${money(downsideLow)}–${money(
      downsideHigh
    )}.${
      hasFavorable
        ? ` Favorable operating relief (reported separately, never netted against downside) is ${money(favorableLow)}–${money(favorableHigh)}.`
        : ""
...
Generated by GroundSense · Report version brief-v8-vp-ready · ${monitoredCount} sources monitored · Next brief: ${datePlusDays(
    ...
    // Label the brief by its GENERATION date. The previous "Week of <today>" mislabeled the
    // generation day as a week start and could read as a future week (UTC/locale drift).
    // "Generated Mon D, YYYY" is unambiguous and never future-looking.
    const title = `Executive Intelligence Brief · Generated ${new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`;

    const summary = `${company.name} has ${risks.length} ranked risks requiring action. Modeled downside exposure is ${money(
      downsideLow
    )}–${money(downsideHigh)}.${
      hasFavorable
        ? ` Favorable operating relief (reported separately) is ${money(favorableLow)}–${money(favorableHigh)}.`
        : ""
    } Top upside is ${money(totalOppLow)}–${money(totalOppHigh)}.`;
